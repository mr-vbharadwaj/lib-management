...
def returnn():
    global SubmitBtn, labelFrame, lb1, bookInfo1, quitBtn, Varun, Canvas1, status
    ...
    issueSql = "delete from " + issueTable + " where bid = '" + str(bid) + "'"

    print(bid in allBid)
    print(status)
    updateStatus = "update " + bookTable + " set status = 'avail' where bid = '" + str(bid) + "'"
    try:
...
def returnBook():
    global bookInfo1, SubmitBtn, quitBtn, Canvas1, con, cur, Varun, labelFrame, lb1
    ...
    SubmitBtn = Button(Varun, text="Return", bg='#d1ccc0', fg='black', command=returnn)
